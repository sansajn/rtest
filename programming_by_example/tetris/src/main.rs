// tetris implementation from *Rust Programming by Example* book, chapter 2
...
impl Tetris {
	...
	fn check_lines(&mut self) {
		...
		let mut y = 0;
		while y < self.game_map.len() {  // TODO: ako sa toto riesi idomickym kodom ? (odstranenie prvku vektora ktorym prave iterujem (remove_if?))
...
struct Tetrimino {  // tetris element
	states: States,
	x: isize,
	y: usize,
	current_state: u8
...
impl Tetrimino {
	...
	fn test_position(&self, game_map: &[Vec<u8>], tmp_state: usize, x: isize, 
		y: usize) -> bool {
...
fn print_game_information(tetris: &Tetris) {
	...
	let mut new_highhest_lines_sent = true;
	if let Some((mut highscores, mut lines_sent)) = load_highscores_and_lines() {
		...
		new_highhest_lines_sent = update_vec(&mut lines_sent, tetris.nb_lines);
		if new_highest_highscore || new_highhest_lines_sent {
	...
	println!("Number of lines: {}{}", 
		tetris.nb_lines,
		if new_highhest_lines_sent { " [NEW HIGHSCORE]"} else { "" });
